    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <form method="POST" action="<?= url('/sucursales/editar?id=' . $branch['id']) ?>">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Color <i class="fas fa-palette text-gray-400 ml-1"></i></label>
                    <div class="flex items-center space-x-3">
                        <input type="color" name="color" value="<?= e($branch['color'] ?? '#3B82F6') ?>" id="colorPicker"
                               class="h-10 w-20 border border-gray-300 rounded-lg cursor-pointer">
                        <span class="text-sm text-gray-500">Color para identificar esta sucursal en el calendario</span>
                    </div>
                </div>
